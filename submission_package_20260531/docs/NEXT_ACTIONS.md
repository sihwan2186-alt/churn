...
| 우선순위 | 작업 | 이유 |
| --- | --- | --- |
| 높음 | `CHURN_DATA_MODEL_DEFENSE.md`로 예상 질문 10분 리허설 | 낮은 precision과 class imbalance 질문에 대비 |
| 높음 | `FINAL_REPORT.md`의 결론 문장을 PPT 결론과 동일하게 맞춤 | 보고서와 발표 간 메시지 충돌 방지 |
...
이 프로젝트는 참고 논문의 불균형 churn 예측 baseline을 재현한 뒤, ZIP 포함 여부, threshold, 비용 구조, 캠페인 예산, segment, calibration, 통계 검정을 추가해 **운영 목적별 모델 선택 프레임워크**를 제시한 프로젝트로 정리하면 가장 안전하다.
