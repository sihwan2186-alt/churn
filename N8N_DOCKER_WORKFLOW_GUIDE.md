...
```powershell
Invoke-RestMethod -Method Post http://localhost:8000/run/full-reproduction -Body '{}' -ContentType 'application/json'
```

Phase 8 통계 검정만 실행:

```powershell
Invoke-RestMethod -Method Post http://localhost:8000/run/statistical-validation -Body '{}' -ContentType 'application/json'
```
...
- workflow JSON의 runner URL은 `http://host.docker.internal:8000`이다. Windows Docker Desktop 환경에서 n8n 컨테이너가 host에 공개된 runner 포트로 접근하기 위한 설정이다.
